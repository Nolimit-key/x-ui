...
func (p *process) refreshVersion() {
	cmd := exec.Command(GetBinaryPath(), "-phiên bản")
	data, err := cmd.Output()
	if err != nil {
		p.version = "Không xác định"
	} else {
...
func (p *process) Start() (err error) {
	if p.IsRunning() {
		return errors.New("xray đã chạy")
	}
...
func (p *process) Stop() error {
	if !p.IsRunning() {
		return errors.New("xray không chạy")
	}
...
func (p *process) GetTraffic(reset bool) ([]*Traffic, error) {
	if p.apiPort == 0 {
		return nil, common.NewError("cổng api xray sai:", p.apiPort)
	}
